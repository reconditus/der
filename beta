...
-- Money Display
        local money_txt = library:create("Text", {
            Text = "Money: $1000", Parent = holder, Visible = true, Transparency = 1, Color = Color3.fromRGB(46, 204, 113), 
            Size = 14, Center = false, Outline = true, Font = Drawing.Fonts.Plex, 
            Position = UDim2.new(0, 20, 0, 35), ZIndex = 15
        })
        -- // 2. Game State & Config //
        plinko_obj.config = {
            money = 1000, -- Added Money
...
-- Cleanup old objects
        ...
        function plinko_obj:SpawnBall()
            if plinko_obj.config.money < plinko_obj.config.bet then return end -- Check funds
            
            plinko_obj.config.money = plinko_obj.config.money - plinko_obj.config.bet
            money_txt.Text = "Money: $"..tostring(plinko_obj.config.money)
            ...
            table.insert(p_state.balls, {
                ...
                vel = Vector2.new(0, 0), active = true, bet = plinko_obj.config.bet
            })
        end
            local ball = CreateObj("Square", {
                Size = UDim2.new(0, 6, 0, 6), Color = Color3.fromRGB(255, 60, 60),
                    ...
                    if b.rel_pos.Y > finish_y then
                        ...
                        for _, slot in pairs(p_state.slots) do
                            if math.abs(b.rel_pos.X - slot.rel_pos.X) < (slot.width/2) then
                                local win = b.bet * slot.val
                                plinko_obj.config.money = plinko_obj.config.money + win
                                money_txt.Text = "Money: $"..tostring(plinko_obj.config.money)
                                
                                library:notify({text = string.format("hit %sx | won $%s", slot.val, win), time = 2})
                                break
                            end
                        end
                    end
