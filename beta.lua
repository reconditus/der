...
local FONT_JSON = FONT_DIR .. "/test.json"
local FONT_URL = "https://raw.githubusercontent.com/reconditus/fonts/main/fs-tahoma-8px.ttf"

local function load_menu_font()
    if not isfolder(FONT_ROOT) then
        makefolder(FONT_ROOT)
    end
    if not isfolder(FONT_DIR) then
        makefolder(FONT_DIR)
    end
    if not isfile(FONT_TTF) then
        local ttf = game:HttpGet(FONT_URL)
        writefile(FONT_TTF, ttf)
    end
    ...
    local manifest = {
        ...
        faces = {{
            ...
            style = "normal",
            weight = 400,
            assetId = getcustomasset(FONT_TTF)
        }}
    }

    writefile(FONT_JSON, HttpService:JSONEncode(manifest))

    return Font.new(
        getcustomasset(FONT_JSON),
        Enum.FontWeight.Regular,
        Enum.FontStyle.Normal
    )
end

local menu_font = load_menu_font()
-- // Handler
...
library.font_face = menu_font
library.font_id = Drawing.Fonts.Plex
...
function utility.dragify(object, title)
    ...
    flags[title .. ' position'] = set
    library.flags[title .. ' position'] = object.Position
...
function library:set_open(bool)
    if typeof(bool) == 'boolean' then
        ...
        -- Get target position
        local targetPos = library.flags['window position'] or self.holder.Position
        
        -- ANIMATION: Diagonal Bottom-Left (-10000 Left, +10000 Down)
        local offScreenPos = UDim2.new(targetPos.X.Scale, targetPos.X.Offset - 10000, targetPos.Y.Scale, targetPos.Y.Offset + 10000)
        
        if bool then
            -- // OPENING //
            library:ToggleParticles(true) -- ENABLE PARTICLES
            
            self.holder.Visible = true
            self.holder.Position = offScreenPos
            
            tween.new(self.holder, TweenInfo.new(0.6, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = targetPos}):Play()

            library.open_windows = library.open_windows + 1
            if library.open_windows == 1 then
                ContextActionService:BindAction("Scrolling", function() return Enum.ContextActionResult.Sink end, false, Enum.UserInputType.MouseWheel);
                ContextActionService:BindAction("Input", function() return Enum.ContextActionResult.Sink end, false, Enum.UserInputType.MouseButton1);
                InputService.MouseIconEnabled = false
            end
        else
            -- // CLOSING //
            library:ToggleParticles(false) -- DISABLE PARTICLES (Saves Performance)
            
            tween.new(self.holder, TweenInfo.new(0.6, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {Position = offScreenPos}):Play()
            ...
            library.open_windows = library.open_windows - 1
            if library.open_windows <= 0 then
...
function library.createdropdown(holder, content, flag, callback, default, max, scrollable, scrollingmax, section, sectioncontent)
    ...
    local icon = library:create("Text", {Text = "-", Transparency = 1, Visible = true, Parent = dropdown, Theme = "Text", ZIndex = 16, Position = UDim2.new(1, -13, 0, 0), Font = library.font_id, Size = 13, Outline = true});
            ...
            if max then
                if table.find(chosen, option) then
                    ...
                    for _, opt in next, chosen do
                        ...
                        if utility.textlength(table.concat(textchosen, ", ") .. ", ...", library.font_id, 13).X > (dropdown.AbsoluteSize.X - 6) then
                            cutobject = true
            ...
            if max then
                if table.find(chosen, option) then
                    ...
                    for _, opt in next, chosen do
                        ...
                        if utility.textlength(table.concat(textchosen, ", ") .. ", ...", library.font_id, 13).X > (list.AbsoluteSize.X - 6) then
                            cutobject = true
                            table.remove(textchosen, #textchosen)
                        end
                    end
...
function library.createslider(cfg)
    ...
    --
    if name then
        local slider_title = library:create("Text", {Text = name, Parent = holder, Visible = true, Transparency = 1, Theme = "Text", Size = 13, Center = false, Outline = false, Font = library.font_id, Position = UDim2.new(0,20,0,-2), ZIndex = 30});
    end
    --
    ...
    --
    local slider_value = library:create("Text", {Text = text, Parent = slider_fill, Visible = true, Transparency = 1, Theme = "Text", Size = 13, Center = true, Outline = true, Font = library.font_id, Position = UDim2.new(1,0,0.5,-2), ZIndex = 31});
    --
            ...
            if max then
                if table.find(chosen, option) then
                    ...
                    for _, opt in next, chosen do
                        ...
                        if utility.textlength(table.concat(textchosen, ", ") .. ", ...", library.font_id, 13).X > (dropdown.AbsoluteSize.X - 18) then
                            cutobject = true
                            table.remove(textchosen, #textchosen)
                        end
                    end
...
function library.object_colorpicker(default, defaultalpha, parent, count, flag, callback, offset)
    ...
    local animation_rainbow = library:create("Text", {
        ...
        Font = library.font_id,
        Size = 13,
        Position = UDim2.new(0.5, -utility.textlength("rainbow", library.font_id, 13).X - 17, 0.5, -60),
        Center = true,
        Theme = "Text",
        ZIndex = 26,
        Outline = false,
    ...
    local animation_lerp = library:create("Text", {
        ...
        Font = library.font_id,
        Size = 13,
        Position = UDim2.new(0.5, utility.textlength("lerp", library.font_id, 13).X - 42, 0.5, -62),
        Center = false,
        Theme = "Text",
        ZIndex = 26,
        Outline = false,
    ...
    local animation_fade = library:create("Text", {
        ...
        Font = library.font_id,
        Size = 13,
        Position = UDim2.new(0.5, utility.textlength("fade", library.font_id, 13).X + 17, 0.5, -62),
        Center = false,
        Theme = "Text",
        ZIndex = 26,
        Outline = false,
    ...
    local rainbow_button = library:create("Square", {Parent = animationpage, Size = UDim2.new(0, utility.textlength("rainbow", library.font_id, 13).X,0,utility.textlength("rainbow", library.font_id, 13).Y +2), Position = UDim2.new(0.5, -utility.textlength("rainbow", library.font_id, 13).X - 41, 0.5, -62), Color = Color3.fromRGB(0,0,0), Thickness = 1, Transparency = 0, Filled = false, ZIndex = 21});
    local lerp_button = library:create("Square", {Parent = animationpage, Size = UDim2.new(0, utility.textlength("lerp", library.font_id, 13).X,0,utility.textlength("lerp", library.font_id, 13).Y +2), Position = UDim2.new(0.5, utility.textlength("lerp", library.font_id, 13).X - 42, 0.5, -62), Color = Color3.fromRGB(0,0,0), Thickness = 1, Transparency = 0, Filled = false, ZIndex = 21});
    local fade_button = library:create("Square", {Parent = animationpage, Size = UDim2.new(0, utility.textlength("fade", library.font_id, 13).X,0,utility.textlength("fade", library.font_id, 13).Y +2), Position = UDim2.new(0.5, utility.textlength("fade", library.font_id, 13).X + 17, 0.5, -62), Color = Color3.fromRGB(0,0,0), Thickness = 1, Transparency = 0, Filled = false, ZIndex = 21});
    local disable_button = library:create("Square", {Parent = animationpage, Size = UDim2.new(0, utility.textlength("disabled", library.font_id, 13).X,0,utility.textlength("disabled", library.font_id, 13).Y +2), Position = UDim2.new(0.5, -26, 0.5, -82), Color = Color3.fromRGB(0,0,0), Thickness = 1, Transparency = 0, Filled = false, ZIndex = 21});
...
-- // Particle Logic (Updated with 200ms Fade In + Fade Out) //
    library.particle_cache = { dots = {}, lines = {}, bg = nil, connection = nil, global_alpha = 0, fade_id = 0 }
    
    function library:InitParticles()
        local VP = workspace.CurrentCamera.ViewportSize
        -- 1. Dark Background
        local bg = drawing:new("Square")
        bg.Visible = false; bg.Filled = true; bg.Thickness = 0; bg.Color = Color3.fromRGB(10, 10, 10); bg.Transparency = 0.4; bg.Size = Vector2.new(VP.X, VP.Y); bg.Position = Vector2.new(0,0); bg.ZIndex = 0
        library.particle_cache.bg = bg

        -- 2. Create Dots
        for i = 1, 40 do
            local dot = drawing:new("Square")
            dot.Visible = false; dot.Filled = true; dot.Thickness = 0; dot.Color = Color3.fromRGB(255, 255, 255); dot.Size = Vector2.new(3, 3); dot.ZIndex = 1
            local pos = Vector2.new(math.random(0, VP.X), math.random(0, VP.Y))
            local vel = Vector2.new(math.random() * 2 - 1, math.random() * 2 - 1) * 0.5
            table.insert(library.particle_cache.dots, {Obj = dot, Pos = pos, Vel = vel})
        end

        -- 3. Create Line Pool
        for i = 1, 800 do 
            local line = drawing:new("Line")
            line.Visible = false; line.Color = Color3.fromRGB(255, 255, 255); line.Thickness = 1; line.Transparency = 0.5; line.ZIndex = 0
            table.insert(library.particle_cache.lines, line)
        end
    end

    function library:ToggleParticles(state)
        local cache = library.particle_cache
        local VP = workspace.CurrentCamera.ViewportSize
        local targetBgAlpha = 0.4 -- CONFIG: Background Darkness
        
        -- Create a unique ID for this specific toggle action to prevent animation conflicts
        local current_fade_id = tick()
        cache.fade_id = current_fade_id

        if state then
            -- // OPENING (Fade In) //
            cache.global_alpha = 0 -- Start invisible
            
            if cache.connection then cache.connection:Disconnect() end
            
            -- Show Elements (Visually they are hidden by global_alpha = 0)
            if cache.bg then cache.bg.Visible = true; cache.bg.Size = Vector2.new(VP.X, VP.Y) end
            for _, dot in pairs(cache.dots) do dot.Obj.Visible = true end
            
            -- 1. Animation Loop (Fade In: 0 -> 1 in 200ms)
            task.spawn(function()
                local duration = 0.2 -- 200ms fade in
                local startTime = tick()
                
                while tick() - startTime < duration do
                    if cache.fade_id ~= current_fade_id then return end -- Stop if user closed menu mid-fade
                    
                    local progress = (tick() - startTime) / duration
                    cache.global_alpha = math.clamp(progress, 0, 1)
                    game:GetService("RunService").RenderStepped:Wait()
                end
                
                if cache.fade_id == current_fade_id then 
                    cache.global_alpha = 1 
                end
            end)

            -- 2. Logic Loop (Movement & Connections)
            cache.connection = game:GetService("RunService").RenderStepped:Connect(function()
                local lineIdx = 1
                local maxDist = 150
                VP = workspace.CurrentCamera.ViewportSize
                if cache.bg then 
                    cache.bg.Size = VP 
                    cache.bg.Transparency = targetBgAlpha * cache.global_alpha
                end

                for i, dot in ipairs(cache.dots) do
                    -- Movement
                    dot.Pos = dot.Pos + dot.Vel
                    if dot.Pos.X < 0 or dot.Pos.X > VP.X then dot.Vel = Vector2.new(-dot.Vel.X, dot.Vel.Y) end
                    if dot.Pos.Y < 0 or dot.Pos.Y > VP.Y then dot.Vel = Vector2.new(dot.Vel.X, -dot.Vel.Y) end
                    dot.Obj.Position = dot.Pos
                    
                    -- Apply Global Alpha
                    dot.Obj.Transparency = 1 * cache.global_alpha

                    -- Lines
                    for j = i + 1, #cache.dots do
                        local other = cache.dots[j]
                        local dist = (other.Pos - dot.Pos).Magnitude
                        if dist < maxDist then
                            local line = cache.lines[lineIdx]
                            if line then
                                line.Visible = true
                                line.From = dot.Pos + Vector2.new(1,1)
                                line.To = other.Pos + Vector2.new(1,1)
                                -- Fade based on Distance AND Global Alpha
                                line.Transparency = (1 - (dist / maxDist)) * cache.global_alpha
                                lineIdx = lineIdx + 1
                            end
                        end
                    end
                ...
                for k = lineIdx, #cache.lines do
                    cache.lines[k].Visible = false
                end
            end)

        else
            -- // CLOSING (Fade Out: 1 -> 0 in 300ms) //
            task.spawn(function()
                local duration = 0.3 
                local startTime = tick()
                local startAlpha = cache.global_alpha -- Fade from current alpha (in case it wasn't fully visible yet)
                
                while tick() - startTime < duration do
                    if cache.fade_id ~= current_fade_id then return end -- Stop if user re-opened menu
                    
                    local progress = (tick() - startTime) / duration
                    cache.global_alpha = math.clamp(startAlpha * (1 - progress), 0, 1)
                    
                    game:GetService("RunService").RenderStepped:Wait()
                end
                
                -- Final Cleanup
                if cache.fade_id ~= current_fade_id then return end
                if cache.connection then cache.connection:Disconnect() end
                if cache.bg then cache.bg.Visible = false end
                for _, dot in pairs(cache.dots) do dot.Obj.Visible = false end
                for _, line in pairs(cache.lines) do line.Visible = false end
            end)
        end
    end
--
function library:new(cfg)
if not library.particle_cache.bg then library:InitParticles() end
    -- // Configuration
    ...
    --
    local window_title = library:create("Text", {Text = name_white, Parent = window_outline, Visible = true, Transparency = 1, Theme = "Accent", Size = 13, Center = false, Outline = false, Font = library.font_id, Position = UDim2.new(0,10,0,7), ZIndex = 13});
    local window_title_accent = library:create("Text", {Text = name_color, Parent = window_outline, Visible = true, Transparency = 1, Theme = "Text", Size = 13, Center = false, Outline = false, Font = library.font_id, Position = UDim2.new(0,utility.textlength(name_white, library.font_id, 13).X + 10,0,7), ZIndex = 13});
    --
    ...
    function window:page(cfg)
        ...
        --
        local button_title = library:create("Text", {Text = name, Parent = button_holder, Visible = true, Transparency = 1, Theme = "Text", Size = 13, Center = true, Outline = true, Font = library.font_id, Position = UDim2.new(0.5,0,0,9), ZIndex = 13});
        ...
        function page:section(cfg)
            ...
            --
            local section_title_cover = library:create("Square", {Parent = section_holder, Visible = true, Transparency = 1, Color = Color3.fromRGB(19,19,19), Size = UDim2.new(0,utility.textlength(name, library.font_id, 13).X + 2,0,4), Position = UDim2.new(0,10,0,-1), Thickness = 1, Filled = true, ZIndex = 14})
            local section_title = library:create("Text", {Text = name, Parent = section_holder, Visible = true, Transparency = 1, Theme = "Text", Size = 13, Center = false, Outline = false, Font = library.font_id, Position = UDim2.new(0,11,0,-8), ZIndex = 14});
            --
            ...
            function section:toggle(cfg)
                ...
                local toggle_title = library:create("Text", {Text = name, Parent = holder, Visible = true, Transparency = 1, Theme = risky and "Risky Text" or "Text", Size = 13, Center = false, Outline = false, Font = library.font_id, Position = UDim2.new(0,20,0,-5), ZIndex = 14});
            ...
            function section:divider(cfg)
                ...
                local title = library:create("Text", {Text = name, Parent = holder, Visible = true, Transparency = 1, Color = Color3.fromRGB(100, 100, 100), Size = 13, Center = false, Outline = false, Font = library.font_id, Position = UDim2.new(0,19,0,-3), ZIndex = 14});
                local div = library:create("Square", { Parent = holder, Visible = true, Transparency = 1, Color = Color3.fromRGB(100, 100, 100), Size = UDim2.new(1, (- utility.textlength(name, library.font_id, 13).X) - 45 + offset, 0, 1), Position = UDim2.new(0, 28 + utility.textlength(name, library.font_id, 13).X - offset, 0, 5), Thickness = 1, Filled = true, ZIndex = 14 })
                --
                if size == "auto" then
            ...
            function section:slider(cfg)
                ...
                if name then
                    local slider_title = library:create("Text", {Text = name, Parent = holder, Visible = true, Transparency = 1, Theme = "Text", Size = 13, Center = false, Outline = false, Font = library.font_id, Position = UDim2.new(0,20,0,-2), ZIndex = 14})
                end
                ...
                if not range then
                    ...
                    local slider_value = library:create("Text", {Text = text, Parent = slider_fill, Visible = true, Transparency = 1, Theme = "Text", Size = 13, Center = true, Outline = true, Font = library.font_id, Position = UDim2.new(1,0,0.5,-2), ZIndex = 15});
                    ...
                    local min_value_text = library:create("Text", {Text = tostring(range[1]) .. suffix, Parent = min_handle, Visible = true, Transparency = 1, Theme = "Text", Size = 13, Center = true, Outline = true, Font = library.font_id, Position = UDim2.new(0.5, 0, 0.5, 0), ZIndex = 15});
                    local max_value_text = library:create("Text", {Text = tostring(range[2]) .. suffix, Parent = max_handle, Visible = true, Transparency = 1, Theme = "Text", Size = 13, Center = true, Outline = true, Font = library.font_id, Position = UDim2.new(0.5, 0, 0.5, 0), ZIndex = 15});
                ...
                if allow then
                    local slider_question = library:create("Text", {Text = "?", Parent = holder, Visible = true, Transparency = 1, Theme = "Text", Size = 13, Center = false, Outline = false, Font = library.font_id, Position = UDim2.new(1,-36,0,-2), ZIndex = 14});
                    ...
                    local fadetext = library:create("Text", {Text = "fading", Parent = slider_button, Visible = true, Transparency = 1, Theme = "Text", Size = 13, Center = true, Outline = false, Font = library.font_id, Position = UDim2.new(0.5, 0, 0, 1), ZIndex = 29});
            ...
            function section:button(cfg)
                ...
                local icon = library:create("Text", {Text = name, Transparency = 1, Visible = true, Parent = ButtonFrame, Theme = "Text", ZIndex = 16, Center = true, Position = UDim2.new(0.5, 0, 0, 1), Font = library.font_id, Size = 13, Outline = true});
                ...
                function button_tbl:button(cfg)
                    ...
                    local icon = library:create("Text", {Text = name, Transparency = 1, Visible = true, Parent = ButtonFrame_2, Theme = "Text", ZIndex = 16, Center = true, Position = UDim2.new(0.5, 0, 0, 1), Font = library.font_id, Size = 13, Outline = true});
            ...
            function section:textbox(cfg)
                ...
                local text = library:create("Text", {Text = default, Transparency = 1, Visible = true, Parent = textbox, Theme = "Text", ZIndex = 14, Center = true, Position = UDim2.new(0.5, 0, 0, 1), Font = library.font_id, Size = 13, Outline = true});
                local placeholder = library:create("Text", {Text = placeholder, Transparency = 1, Visible = true, Parent = textbox, Theme = "Un-Selected_Text", ZIndex = 14, Center = true, Position = UDim2.new(0.5, 0, 0, 1), Font = library.font_id, Size = 13, Outline = true});
            ...
            function section:preview(cfg)
                ...
                local esp_health_text = library:create("Text", {Text = tostring("<- "..healthamount), Parent = esp_health_bar, Visible = true, Transparency = 1, Color = maincolor, Size = 13, Center = false, Outline = true, Font = library.font_id, Position = UDim2.new(1,0,0,0), ZIndex = 16});

                local esp_name = library:create("Text", {Text = "player", Parent = preview_frame, Visible = false, Transparency = 1, Color = Color3.fromRGB(255, 255, 255), Size = 13, Center = true, Outline = true, Font = library.font_id, Position = UDim2.new(0, 110, 0, 3), ZIndex = 16});
                local esp_distance = library:create("Text", {Text = "0 meters", Parent = preview_frame, Visible = false, Transparency = 1, Color = Color3.fromRGB(255, 255, 255), Size = 13, Center = true, Outline = true, Font = library.font_id, Position = UDim2.new(0, 110, 0, 260), ZIndex = 16});
                local esp_weapon = library:create("Text", {Text = "weapon", Parent = preview_frame, Visible = false, Transparency = 1, Color = Color3.fromRGB(255, 255, 255), Size = 13, Center = true, Outline = true, Font = library.font_id, Position = UDim2.new(0, 110, 0, 270), ZIndex = 16});
                --
            ...
            function section:infographic(cfg)
                ...
                --
                player_data = library:create("Text", {Text = info, Parent = holder, Visible = true, Transparency = 1, Theme = "Text", Size = 13, Center = false, Outline = false, Font = library.font_id, Position = UDim2.new(0, 100, 0, 30), ZIndex = 14});
        ...
        function page:multisection(cfg)
            ...
            function multisection:section(cfg)
                ...
                --
                local button_title = library:create("Text", {Text = name, Parent = button_holder, Visible = true, Transparency = 1, Theme = "Un-Selected_Text", Size = 13, Center = true, Outline = true, Font = library.font_id, Position = UDim2.new(0.5,0,0,2), ZIndex = 15});
                ...
                function section:divider(cfg)
                    ...
                    --local div = library:create("Square", { Parent = holder, Visible = true, Transparency = 1, Color = Color3.fromRGB(100, 100, 100), Size = UDim2.new(0, 6, 0, 1), Position = UDim2.new(0, 0, 0, 3), Thickness = 1, Filled = true, ZIndex = 14 })
                    local title = library:create("Text", {Text = name, Parent = holder, Visible = true, Transparency = 1, Color = Color3.fromRGB(100, 100, 100), Size = 13, Center = false, Outline = false, Font = library.font_id, Position = UDim2.new(0,19,0,-3), ZIndex = 14});
                    local div = library:create("Square", { Parent = holder, Visible = true, Transparency = 1, Color = Color3.fromRGB(100, 100, 100), Size = UDim2.new(1, (- utility.textlength(name, library.font_id, 13).X) - 45, 0, 1), Position = UDim2.new(0, 30 + utility.textlength(name, library.font_id, 13).X, 0, 5), Thickness = 1, Filled = true, ZIndex = 14 })
                    --
                    if size == "auto" then
                        section_holder.Size = UDim2.new(1,0,0,section_content.AbsoluteContentSize + 5);
                    end;
    ...
    function window:player_list(cfg)
        ...
        -- // Drawings
        local list_holder = library:create("Square", { Visible = false, Transparency = 1, Color = Color3.fromRGB(13,13,13), Size = UDim2.new(0,500,0,265), Position = UDim2.new(0, 860, 0, 268), Thickness = 1, Filled = true, ZIndex = 14}) do
        ...
        local list_title = library:create("Text", {Text = "players", Parent = list_holder, Visible = true, Transparency = 1, Theme = "Text", Size = 14, Center = true, Outline = false, Font = library.font_id, Position = UDim2.new(0.5,0,0,5), ZIndex = 14});
        ...
        --
        local player_data = library:create("Text", {Text = "...", Parent = list_holder, Visible = true, Transparency = 1, Theme = "Text", Size = 13, Center = false, Outline = false, Font = library.font_id, Position = UDim2.new(0,390,0,120), ZIndex = 14});
        --
        ...
        local icon = library:create("Text", {Text = "priority", Transparency = 1, Visible = true, Parent = PriorityFrame, Theme = "Text", ZIndex = 16, Center = true, Position = UDim2.new(0.5, 0, 0, 1), Font = library.font_id, Size = 13, Outline = true});
        ...
        local friendicon = library:create("Text", {Text = "friendly", Transparency = 1, Visible = true, Parent = FriendFrame, Theme = "Text", ZIndex = 16, Center = true, Position = UDim2.new(0.5, 0, 0, 1), Font = library.font_id, Size = 13, Outline = true});
        ...
        function playerlist:setopen(bool)
            ...
            -- Get position
            local targetPos = library.flags['player list position'] or list_holder.Position
            -- 10,000 pixels LD
            local offScreenPos = UDim2.new(targetPos.X.Scale, targetPos.X.Offset - 10000, targetPos.Y.Scale, targetPos.Y.Offset + 10000)

            if bool then
                list_holder.Visible = true
                list_holder.Position = offScreenPos
                tween.new(list_holder, TweenInfo.new(0.6, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = targetPos}):Play()
                ...
                tween.new(list_holder, TweenInfo.new(0.6, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {Position = offScreenPos}):Play()
                
                task.delay(0.6, function()
                    if not playerlist.open then
                        list_holder.Visible = false
                    end
                end)
    ...
    function window:server_list(cfg)
        ...
        --
        local list_title = library:create("Text", {Text = "servers", Parent = list_holder, Visible = true, Transparency = 1, Theme = "Text", Size = 14, Center = true, Outline = false, Font = library.font_id, Position = UDim2.new(0.5,0,0,5), ZIndex = 14});
        local assent = library:create("Square", {Parent = list_holder, Visible = true, Transparency = 1, Theme = "Accent", Size = UDim2.new(1,-30,0,2), Position = UDim2.new(0,16,0,25), Thickness = 1, Filled = true, ZIndex = 14});
        --
        local list_inline = library:create("Square", {Parent = list_holder, Visible = true, Transparency = 1, Color = Color3.fromRGB(13,13,13), Size = UDim2.new(0, 340, 0, (max * 20)), Position = UDim2.new(0, 20, 0, 45), Thickness = 1, Filled = true, ZIndex = 14}) do
            local outline = library:outline(list_inline, Color3.fromRGB(37,37,37), 14);
        ...
        local connect_frame = library:create("Square", { Filled = true, Visible = true, Thickness = 0, Color = Color3.fromRGB(25,25,25), Transparency = 0.2, Size = UDim2.new(0,60,0,17), Position = UDim2.new(0, 20, 0, 250), ZIndex = 14, Parent = list_holder }) do
            ...
            local text = library:create("Text", {Text = "connect", Transparency = 1, Visible = true, Parent = connect_frame, Theme = "Text", ZIndex = 16, Center = true, Position = UDim2.new(0.5, 0, 0, 1), Font = library.font_id, Size = 13, Outline = true});
        ...
        local sort_frame = library:create("Square", { Filled = true, Visible = true, Thickness = 0, Color = Color3.fromRGB(25,25,25), Transparency = 0.2, Size = UDim2.new(0,120,0,17), Position = UDim2.new(0, 240, 0, 250), ZIndex = 14, Parent = list_holder }) do
            ...
            sort_text = library:create("Text", {Text = "players ascending", Transparency = 0.6, Visible = true, Parent = sort_frame, Theme = "Text", ZIndex = 16, Center = true, Position = UDim2.new(0.5, 0, 0, 1), Font = library.font_id, Size = 13, Outline = true});
        ...
        local refresh_frame = library:create("Square", { Filled = true, Visible = true, Thickness = 0, Color = Color3.fromRGB(25,25,25), Transparency = 0.2, Size = UDim2.new(0,60,0,17), Position = UDim2.new(0, 85, 0, 250), ZIndex = 14, Parent = list_holder }) do
            ...
            library:create("Text", {Text = "refresh", Transparency = 1, Visible = true, Parent = refresh_frame, Theme = "Text", ZIndex = 16, Center = true, Position = UDim2.new(0.5, 0, 0, 1), Font = library.font_id, Size = 13, Outline = true});
        ...
        local textbox_frame = library:create("Square", { ZIndex = 14, Transparency = 0.2, Size = UDim2.new(0,90,0,17),Parent = list_holder, Thickness = 1, Position = UDim2.new(0, 386, 0, 150)}) do
            ...
            local text = library:create("Text", {Text = '', Transparency = 1, Visible = true, Parent = textbox, Theme = "Text", ZIndex = 14, Center = true, Position = UDim2.new(0.5, 0, 0, 1), Font = library.font_id, Size = 13, Outline = true});
            local placeholder = library:create("Text", {Text = 'username or id', Transparency = 1, Visible = true, Parent = textbox, Theme = "Un-Selected_Text", ZIndex = 14, Center = true, Position = UDim2.new(0.5, 0, 0, 1), Font = library.font_id, Size = 13, Outline = true});
        ...
        local button3 = library:create("Square", { Filled = true, Visible = true, Thickness = 0, Color = Color3.fromRGB(25,25,25), Transparency = 0.2, Size = UDim2.new(0,90,0,17), Position = UDim2.new(0, 386, 0, 172), ZIndex = 14, Parent = list_holder }) do
            ...
            local text = library:create("Text", {Text = "scan", Transparency = 1, Visible = true, Parent = button3, Theme = "Text", ZIndex = 16, Center = true, Position = UDim2.new(0.5, 0, 0, 1), Font = library.font_id, Size = 13, Outline = true});
        ...
        local currentserv_text = library:create("Text", {Text = "current server", Transparency = 0.6, Visible = true, Parent = list_holder, Theme = "Text", ZIndex = 16, Center = true, Position = UDim2.new(0, 432, 0, 230), Font = library.font_id, Size = 13, Outline = true});
        ...
        local button1 = library:create("Square", { Filled = true, Visible = true, Thickness = 0, Color = Color3.fromRGB(25,25,25), Size = UDim2.new(0,40,0,17), Position = UDim2.new(0, 386, 0, 250), ZIndex = 14, Parent = list_holder }) do
            ...
            local text = library:create("Text", {Text = "rejoin", Transparency = 1, Visible = true, Parent = button1, Theme = "Text", ZIndex = 16, Center = true, Position = UDim2.new(0.5, 0, 0, 1), Font = library.font_id, Size = 13, Outline = true});
        ...
        local button2 = library:create("Square", { Filled = true, Visible = true, Thickness = 0, Color = Color3.fromRGB(25,25,25), Size = UDim2.new(0,40,0,17), Position = UDim2.new(0, 436, 0, 250), ZIndex = 14, Parent = list_holder }) do
            ...
            library:create("Text", {Text = "copy", Transparency = 1, Visible = true, Parent = button2, Theme = "Text", ZIndex = 16, Center = true, Position = UDim2.new(0.5, 0, 0, 1), Font = library.font_id, Size = 13, Outline = true});
        ...
        function serverlist:setopen(bool)
            ...
            -- Get position
            local targetPos = library.flags['server list position'] or list_holder.Position
            -- 10,000 pixels LD
            local offScreenPos = UDim2.new(targetPos.X.Scale, targetPos.X.Offset - 10000, targetPos.Y.Scale, targetPos.Y.Offset + 10000)
...
-- // New Standalone ESP Preview Window (Fixed Position & Saving) //
    -- // New Standalone ESP Preview Window (Fixed: Resets if lost off-screen) //
    function window:esp_preview_window(cfg)
        local preview_obj = { open = false }
        local flag = cfg.flag or utility.nextflag()
        ...
        -- Default Spawn: To the right of the main UI
        local default_pos = UDim2.new(0, 1380, 0, 268) 

        -- 1. Create the Window Frame
        local preview_holder = library:create("Square", {
            Visible = false, 
            Transparency = 1, 
            Color = Color3.fromRGB(13,13,13), 
        ...
        local outline = library:outline(preview_holder, Color3.fromRGB(37,37,37), 14)
        library:outline(outline, Color3.fromRGB(0,0,0), 14)

        -- Make draggable (Saves to flag)
        utility.dragify(preview_holder, flag)

        -- Title & Accent Line
        library:create("Text", {
            Text = "esp preview", 
            Parent = preview_holder, 
            Visible = true, 
            Transparency = 1, 
            Theme = "Text", 
            Size = 14, 
        ...
        local esp_head = library:create("Square", {Parent = preview_frame, Size = UDim2.new(0, 44, 0, 39), Position = UDim2.new(0, 86 + x_offset, 0, 45 + y_offset), Color = Color3.fromRGB(245, 245, 245), Thickness = 1, Filled = true, ZIndex = 16})
        library:outline(esp_head, Color3.fromRGB(0, 0, 0), 15)

        local esp_torso = library:create("Square", {Parent = preview_frame, Size = UDim2.new(0, 146, 0, 77), Position = UDim2.new(0, 34 + x_offset, 0, 85 + y_offset), Color = Color3.fromRGB(245, 245, 245), Thickness = 1, Filled = true, ZIndex = 16})
        library:outline(esp_torso, Color3.fromRGB(0, 0, 0), 15)

        local esp_legs = library:create("Square", {Parent = preview_frame, Size = UDim2.new(0, 72, 0, 78), Position = UDim2.new(0, 72 + x_offset, 0, 163 + y_offset), Color = Color3.fromRGB(245, 245, 245), Thickness = 1, Filled = true, ZIndex = 16})
        library:outline(esp_legs, Color3.fromRGB(0, 0, 0), 15)

        -- 4. Draw ESP Visuals
        local esp_bounding_box = library:create("Square", {Visible = false, Parent = preview_frame, Size = UDim2.new(0, 195, 0, 240), Position = UDim2.new(0, 13 + x_offset, 0, 20 + y_offset), Color = Color3.fromRGB(255, 255, 255), Thickness = 1, Filled = false, ZIndex = 16})
        local esp_bounding_box_outline = library:outline(esp_bounding_box, Color3.fromRGB(0, 0, 0), 16)

        local esp_health_bar_outline = library:create("Square", {Visible = false, Parent = preview_frame, Size = UDim2.new(0, 3, 0, 240), Position = UDim2.new(0, 6 + x_offset, 0, 20 + y_offset), Color = Color3.fromRGB(0, 0, 0), Thickness = 1, Filled = true, ZIndex = 16})
        local esp_health_bar_outline_2 = library:outline(esp_health_bar_outline, Color3.new(0,0,0), 16)
        ...
        local esp_name = library:create("Text", {Text = "player", Parent = preview_frame, Visible = false, Transparency = 1, Color = Color3.fromRGB(255, 255, 255), Size = 13, Center = true, Outline = true, Font = library.font_id, Position = UDim2.new(0, 110 + x_offset, 0, 3 + y_offset), ZIndex = 16})
        
        local esp_distance = library:create("Text", {Text = "0 meters", Parent = preview_frame, Visible = false, Transparency = 1, Color = Color3.fromRGB(255, 255, 255), Size = 13, Center = true, Outline = true, Font = library.font_id, Position = UDim2.new(0, 110 + x_offset, 0, 260 + y_offset), ZIndex = 16})
        
        local esp_weapon = library:create("Text", {Text = "weapon", Parent = preview_frame, Visible = false, Transparency = 1, Color = Color3.fromRGB(255, 255, 255), Size = 13, Center = true, Outline = true, Font = library.font_id, Position = UDim2.new(0, 110 + x_offset, 0, 270 + y_offset), ZIndex = 16})
        ...
        function preview_obj:set_color(element, state)
            if element == "box" then esp_bounding_box.Color = state
            elseif element == "box outline" then esp_bounding_box_outline.Color = state
            elseif element == "healthbar outline" then esp_health_bar_outline.Color = state; esp_health_bar_outline_2.Color = state
            elseif element == "name" then esp_name.Color = state
            elseif element == "name outline" then esp_name.OutlineColor = state
            elseif element == "distance" then esp_distance.Color = state
            elseif element == "distance outline" then esp_distance.OutlineColor = state
            elseif element == "weapon" then esp_weapon.Color = state
            elseif element == "weapon outline" then esp_weapon.OutlineColor = state end
        end

        function preview_obj:set_visibility(element, state)
            if element == "box" then esp_bounding_box.Visible = state
            elseif element == "healthbar" then esp_health_bar_outline.Visible = state
            elseif element == "name" then esp_name.Visible = state
            elseif element == "distance" then 
                esp_distance.Visible = state
                if esp_weapon.Visible and not esp_distance.Visible then
                    esp_weapon.Position = UDim2.new(0, 110 + x_offset, 0, 260 + y_offset)
                else
                    esp_weapon.Position = UDim2.new(0, 110 + x_offset, 0, 270 + y_offset)
                end
            elseif element == "weapon" then 
                esp_weapon.Visible = state 
                if esp_weapon.Visible and not esp_distance.Visible then
        ...
        -- 6. SetOpen (Fixed Logic)
        function preview_obj:setopen(bool)
            preview_obj.open = bool
            local ContextActionService = game:GetService("ContextActionService")
            local InputService = game:GetService('UserInputService')

            local targetPos = library.flags[flag .. ' position'] or default_pos
            
            -- SAFETY CHECK: If the target pos is insanely far away (saved while closed), reset it.
            if math.abs(targetPos.Y.Offset) > 2000 or math.abs(targetPos.X.Offset) > 5000 then
                targetPos = default_pos
            end

            -- Animation Offset: -10000 Left, +10000 Down (Diagonal Bottom-Left)
            local offScreenPos = UDim2.new(targetPos.X.Scale, targetPos.X.Offset - 10000, targetPos.Y.Scale, targetPos.Y.Offset + 10000)

            if bool then
                preview_holder.Visible = true
                preview_holder.Position = offScreenPos
                tween.new(preview_holder, TweenInfo.new(0.6, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = targetPos}):Play()
                ...
                tween.new(preview_holder, TweenInfo.new(0.6, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {Position = offScreenPos}):Play()
                
                task.delay(0.6, function()
                    if not preview_obj.open then
    ...
    --
-- // PLINKO WINDOW (Fixed: Removed Double Positioning Logic) //
    function window:plinko_window(cfg)
        local plinko_obj = { open = false }
        local flag = cfg.flag or utility.nextflag()
        
        -- Default Position
        local default_pos = UDim2.new(0, 1380, 0, 600) 
        ...
        local outline = library:outline(holder, Color3.fromRGB(37,37,37), 14)
        library:outline(outline, Color3.fromRGB(0,0,0), 14)
        
        utility.dragify(holder, flag)

        -- Title
        library:create("Text", {
            Text = "plinko", Parent = holder, Visible = true, Transparency = 1, Theme = "Text", 
            Size = 14, Center = true, Outline = false, Font = library.font_id, 
            Position = UDim2.new(0.5,0,0,5), ZIndex = 14
        })
        
        library:create("Square", {
            Parent = holder, Visible = true, Transparency = 1, Theme = "Accent", 
            Size = UDim2.new(1,-30,0,2), Position = UDim2.new(0,16,0,25), 
            Thickness = 1, Filled = true, ZIndex = 14
        })

        -- // 2. Game State & Config //
        plinko_obj.config = {
            rows = 12, spacing = 28, gravity = 700, dampening = 0.6, bet = 100, risk = "Medium",
            multipliers = {
                Low = {5, 3, 1.5, 1, 0.7, 0.7, 1, 1.5, 3, 5},
                Medium = {10, 5, library.font_id, 0.5, 0.2, 0.2, 0.5, library.font_id, 5, 10},
                High = {25, 10, 5, 0, 0, 0, 0, 5, 10, 25}
            }
        }
        local p_state = { pegs = {}, balls = {}, slots = {}, walls = {}, active = false }

        -- // 3. Helper Functions //
        local function CreateObj(type, props)
            local obj = library:create(type, props)
            obj.Parent = holder
            obj.Visible = true 
            return obj
        end

        function plinko_obj:GenerateBoard()
-- Cleanup old objects
            for _, p in pairs(p_state.pegs) do p.obj:Remove() end
            for _, s in pairs(p_state.slots) do s.bg:Remove(); s.text:Remove() end
            
            -- FAIL-SAFE: Check if walls exists before looping
            if p_state.walls then
                for _, w in pairs(p_state.walls) do w:Remove() end
            end

            p_state.pegs = {}
            p_state.slots = {}
            p_state.walls = {} -- Initialize it here for sure
            ...
            local top_wall = CreateObj("Square", { Size = UDim2.new(1, -20, 0, 2), Position = UDim2.new(0, 10, 0, 45), Theme = "Accent", Filled = true, Thickness = 0, ZIndex = 15, Transparency = 1 })
            local left_wall = CreateObj("Square", { Size = UDim2.new(0, 2, 1, -60), Position = UDim2.new(0, 10, 0, 45), Color = wall_color, Filled = true, Thickness = 0, ZIndex = 15, Transparency = 1 })
            local right_wall = CreateObj("Square", { Size = UDim2.new(0, 2, 1, -60), Position = UDim2.new(1, -12, 0, 45), Color = wall_color, Filled = true, Thickness = 0, ZIndex = 15, Transparency = 1 })
            
            table.insert(p_state.walls, top_wall); table.insert(p_state.walls, left_wall); table.insert(p_state.walls, right_wall)

            -- Pegs
            for row = 1, plinko_obj.config.rows do
                local pegs_in_row = row + 2
                local width = (pegs_in_row - 1) * plinko_obj.config.spacing
                local offset_x = center_x - (width / 2)

                for col = 1, pegs_in_row do
                    local rx = offset_x + ((col - 1) * plinko_obj.config.spacing)
                    local ry = start_y + ((row - 1) * plinko_obj.config.spacing)
                    
                    local peg = CreateObj("Square", {
                        Size = UDim2.new(0, 4, 0, 4), Position = UDim2.new(0, rx, 0, ry),
                        Color = Color3.fromRGB(200, 200, 200), Filled = true, Thickness = 0, ZIndex = 16, Transparency = 1
                    })
                    
                    table.insert(p_state.pegs, { obj = peg, rel_pos = Vector2.new(rx, ry), radius = 3 })
                end
            end

            -- Slots
            local current_mults = plinko_obj.config.multipliers[plinko_obj.config.risk] or plinko_obj.config.multipliers.Medium
            local last_row_y = start_y + (plinko_obj.config.rows * plinko_obj.config.spacing)
            local total_width = (plinko_obj.config.rows + 2) * plinko_obj.config.spacing
            local start_x = center_x - (total_width / 2) + (plinko_obj.config.spacing/2)

            for i = 1, plinko_obj.config.rows + 1 do
                local mid = math.ceil((plinko_obj.config.rows + 1)/2)
                local dist = math.abs(i - mid)
                local idx = math.clamp(math.floor(dist/(mid/#current_mults)) + 1, 1, #current_mults)
                local mult = current_mults[idx] or 1

                local sx = start_x + ((i - 1) * plinko_obj.config.spacing)
                
                local color = Color3.fromRGB(80, 80, 200)
                if mult >= 5 then color = Color3.fromRGB(200, 50, 50) 
                elseif mult < 1 then color = Color3.fromRGB(80, 80, 80) end

                local bg = CreateObj("Square", {
                    Size = UDim2.new(0, plinko_obj.config.spacing - 4, 0, 20),
                    Position = UDim2.new(0, sx, 0, last_row_y + 15),
                    Color = color, Filled = true, Thickness = 0, ZIndex = 15, Transparency = 1
                })
                local txt = CreateObj("Text", {
                    Text = tostring(mult).."x", Size = 10, Center = true, Outline = true, Theme = "Text",
                    Position = UDim2.new(0, sx + (plinko_obj.config.spacing/2) - 2, 0, last_row_y + 18), ZIndex = 17, Transparency = 1
                })

                table.insert(p_state.slots, { bg = bg, text = txt, val = mult, rel_pos = Vector2.new(sx, last_row_y + 15), width = plinko_obj.config.spacing })
            end
        end

        function plinko_obj:SpawnBall()
            local ball = CreateObj("Square", {
                Size = UDim2.new(0, 6, 0, 6), Color = Color3.fromRGB(255, 60, 60),
                Filled = true, Thickness = 0, ZIndex = 18, Transparency = 1
            })
            table.insert(p_state.balls, {
                obj = ball, rel_pos = Vector2.new(250 + math.random(-5, 5), 20),
                vel = Vector2.new(0, 0), active = true
            })
        end
        
        function plinko_obj:Clear()
            for _, b in pairs(p_state.balls) do b.obj:Remove() end
            p_state.balls = {}
        end
...
-- // 4. Physics Loop //
        game:GetService("RunService").RenderStepped:Connect(function(dt)
            ...
            -- FIX: Removed manual updates for Pegs, Slots, and Walls.
            -- The library handles their position relative to the window automatically.

            -- Ball Physics
            for i = #p_state.balls, 1, -1 do
                local b = p_state.balls[i]
                if b.active then
                    -- Gravity
                    b.vel = b.vel + Vector2.new(0, plinko_obj.config.gravity * dt)
                    local next_pos = b.rel_pos + (b.vel * dt)

                    -- Wall Collisions
                    if next_pos.X < 15 or next_pos.X > 485 then
                        b.vel = Vector2.new(-b.vel.X * 0.5, b.vel.Y)
                        next_pos = Vector2.new(math.clamp(next_pos.X, 15, 485), next_pos.Y)
                    end

                    -- Peg Collisions
                    for _, peg in pairs(p_state.pegs) do
                        local diff = next_pos - peg.rel_pos
                        local dist = diff.Magnitude
                        if dist < (4 + peg.radius) then
                            local norm = diff.Unit
                            b.vel = b.vel - (2 * b.vel:Dot(norm) * norm)
                            b.vel = b.vel * plinko_obj.config.dampening
                            b.vel = b.vel + Vector2.new(math.random(-30, 30), 0)
                            next_pos = peg.rel_pos + (norm * (4 + peg.radius + 1))
                        end
                    end
                    b.rel_pos = next_pos
                    
                    -- FIX: Update Ball Visual using RELATIVE position only (No win_x/win_y)
                    b.obj.Position = UDim2.new(0, b.rel_pos.X - 4, 0, b.rel_pos.Y - 4)

                    -- Floor / Scoring
                    local finish_y = 50 + (plinko_obj.config.rows * plinko_obj.config.spacing) + 15
                    if b.rel_pos.Y > finish_y then
                        b.active = false
                        b.obj:Remove()
                        table.remove(p_state.balls, i)
                        -- Determine win
                        for _, slot in pairs(p_state.slots) do
                            if math.abs(b.rel_pos.X - slot.rel_pos.X) < (slot.width/2) then
                                library:notify({text = string.format("hit %sx multiplier", slot.val), time = 2})
                                break
                            end
                        end
                    end
        ...
        plinko_obj:GenerateBoard()

        -- // 5. SetOpen //
        function plinko_obj:setopen(bool)
            plinko_obj.open = bool
            local targetPos = library.flags[flag .. ' position'] or default_pos
            if math.abs(targetPos.Y.Offset) > 2000 or math.abs(targetPos.X.Offset) > 5000 then targetPos = default_pos end
            local offScreenPos = UDim2.new(targetPos.X.Scale, targetPos.X.Offset - 10000, targetPos.Y.Scale, targetPos.Y.Offset + 10000)

            if bool then
                holder.Visible = true
                holder.Position = offScreenPos
                tween.new(holder, TweenInfo.new(0.6, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = targetPos}):Play()
                
                library.open_windows = library.open_windows + 1
                if library.open_windows == 1 then
                    game:GetService('UserInputService').MouseIconEnabled = false
                    game:GetService("ContextActionService"):BindAction("Input", function() return Enum.ContextActionResult.Sink end, false, Enum.UserInputType.MouseButton1)
                end
            else
                tween.new(holder, TweenInfo.new(0.6, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {Position = offScreenPos}):Play()
                task.delay(0.6, function() if not plinko_obj.open then holder.Visible = false end end)
                
                library.open_windows = library.open_windows - 1
                if library.open_windows <= 0 then
                    game:GetService('UserInputService').MouseIconEnabled = true
                    game:GetService("ContextActionService"):UnbindAction("Input")
                end
            end
        end
...
function library:notify(info)
    ...
    local name = info.title or info.Title or 'I dont like BBC.. Im too fragile'
    ...
    local background = library:create('Square', {
        Size = UDim2.new(0, utility.textlength(title, library.font_id, 13).X + utility.textlength(name, library.font_id, 13).X + 10, 0, 19);
    ...
    --
    local text = library:create("Text", { Parent = background, Visible = true, Transparency = 1, Theme = "Accent", Size = 13, Text = name, Center = false, Outline = false, Font = library.font_id, Position = UDim2.new(0, 3,0,2), ZIndex = 11})
    local notiftext = library:create("Text", { Text = title, Parent = background, Visible = true, Transparency = 1, Theme = "Text", Size = 13, Center = false, Outline = false, Font = library.font_id, Position = UDim2.new(0, utility.textlength(text.Text, library.font_id, 13).X + 5,0,2), ZIndex = 11})
    --
...
function library:createwatermark()
    ...
    watermark.objects.background = library:create('Square', { Size = UDim2.new(0, utility.textlength(watermark.title, library.font_id, 13).X + 5, 0, 19), Position = UDim2.new(0, 18, 0, 75), Color = Color3.fromRGB(13, 13, 13), ZIndex = 1, Thickness = 1, Filled = true }, true)
    ...
    watermark.objects.text2 = library:create("Text", { Parent = watermark.objects.background, Visible = true, Transparency = 1, Theme = "Accent", Size = 13, Text = 'dreya', Center = false, Outline = false, Font = library.font_id, Position = UDim2.new(0, 3, 0, 2), ZIndex = 11 })
    watermark.objects.text3 = library:create("Text", { Text = watermark.title, Parent = watermark.objects.background, Visible = true, Transparency = 1, Theme = "Text", Size = 13, Center = false, Outline = false, Font = library.font_id, Position = UDim2.new(0, utility.textlength(watermark.objects.text2.Text, library.font_id, 13).X + 5, 0, 2), ZIndex = 11 })
    ...
    game:GetService("RunService").Heartbeat:Connect(function()
        if (tick() - watermark_init) * 1000 > watermark.tickrate then
            ...
            if watermark.objects.background.Visible then
                ...
                local newSize = UDim2.new(0, utility.textlength(title, library.font_id, 13).X + utility.textlength('dreya', library.font_id, 13).X + 10, 0, 19)
                ...
                watermark.objects.text3.Position = UDim2.new(0, utility.textlength(watermark.objects.text2.Text, library.font_id, 13).X + 5, 0, 2)
            end
        end
    end)
